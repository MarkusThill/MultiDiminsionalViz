...
parCoordPlot<-function(data_m, useWhiteBackground, xLab, yLab, alpha, xText, yText, lsize) {

  p <- ggplot(data_m) + 
    geom_line(aes(x = variable, y = value, group = ID), size=lsize, alpha = alpha)
   p<-p +geom_point(aes(x = variable, y = value, group = ID), cex=3, pch='|', alpha = alpha/10.0)
...
remove_outliers <- function(x) {
  y<-x
  fac<-4
  # don't remove, if more than 1% of the observations are affected
