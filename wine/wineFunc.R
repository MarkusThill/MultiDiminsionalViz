...
# Stupid function: already implemented in R
...
parCoordPlotCol<-function(data_m, useWhiteBackground, xLab, yLab, xText, yText, maxLab, minLab, colVec, alphaVec, linesize) {
  p <- ggplot(data_m) + 
    geom_line(aes(x = variable, y = value, group = ID, color=cquality, alpha = cquality), size=linesize)
  p<-p +geom_point(aes(x = variable, y = value, group = ID, color=cquality), cex=6, pch='|', alpha=0.1)
...
remove_outliers <- function(x, perc=0.01) {
  y<-x
  fac<-2
  # don't remove, if more than 2% of the observations are affected
